...
### Разобранная плата приборки

Приборка в разобранном виде — видна печатная плата целиком (слева основной разъём
питание+CAN, справа динамик):

![Разобранная плата приборки SX11-A5](photos/dashboard-disassembled.jpg)

Те же чипы с подписями — по центру **MCU** (Infineon TRAVEO II CYT3DL, большой чип в
корпусе TEQFP), рядом **NOR flash** (одна из двух микросхем S25HL512T, откуда снимаются
дампы):

![Плата приборки с подписанными чипами MCU и NOR flash](photos/dashboard-disassembled-chips.jpg)

> ℹ️ На фото виден **только один** из двух NOR-чипов — он на **лицевой** стороне платы.
> **Второй NOR-чип расположен на обратной стороне** платы (фото обратной стороны с чипом
> нет). Оба вместе (U17 + U18) и составляют 2×64 МБ внешней памяти приборки.

Рядом с MCU (между процессором и шлейфом дисплея) выделена группа неустановленных
контактных площадок:

![Потенциальные отладочные (JTAG) пины на плате приборки](photos/dashboard-disassembled-jtag.jpg)

Это **потенциальные отладочные пины** (посадочное место под гребёнку, разъём не распаян). По
расположению и типу — вероятно **JTAG/SWD-порт MCU**: к ним можно **подпаять JTAG и читать**
подготовленную память приборки через XIP-окно на работающей приборке (метод алжирцев — см.
[docs/INFINEON_REPOS.md](docs/INFINEON_REPOS.md)). Это визуально подтверждает известное со
слов сообщества «на плате JTAG есть, но пины не распаяны». ⚠️ Назначение конкретных площадок
(TCK/TMS/TDI/TDO/…) **требует прозвонки** — по фото не определяется.

Разбор железа и маркировок — в разделе [«Железо»](#железо-подтверждено) ниже.

---

## 🎯 Успешный результат на сегодня: иконки, извлечённые из дампа

Ниже — **реальная графика приборки, прочитанная напрямую из сырого NOR-дампа** (не рендер-
заготовка). Это библиотека телльтейлов, хранящаяся несжатым сырым A8 — её удалось извлечь
**без** таблицы разметки из закрытого MCU и **без** RLAD-декодера:

![Иконки приборки, извлечённые из дампа U17](results/U17_icons_0x28F8000_w48.png)

Видны глазами: медиа/ноты, авто (вид спереди/сзади), спидометры, замки, чашка кофе
(усталость), «P» парковка, MAX/MIN, предупреждения, фары и т.д. Тот же набор
воспроизводится на обоих чипах (U17 и U18).

- **Зона:** ~`0x28F8000..0x2940000` (иногда удобнее стартовать с `0x28F4000`), stride 48 px, формат raw A8.
- **Воспроизвести:** `python tools/extract_raw_a8.py Coolray_25_Dash_U17.bin`
- Полный разбор находки, метод и оговорки — [docs/SUCCESSFUL_FINDINGS.md](docs/SUCCESSFUL_FINDINGS.md);
  вторая галерея (чип U18) — в [`results/`](results/).

> Это первый прямой «прокол» главного барьера проекта: показывает, что **несжатые зоны
> дампа читаются напрямую**. Текст и цветная графика по-прежнему в RLAD — их вслепую не
> взять (нужен diff двух языков или JTAG-съём через XIP, см. ниже).
...
| Внешняя память | **2 × Infineon SEMPER NOR S25HL512T** (позиции U17, U18), по 64 МБ — один чип на лицевой стороне платы, второй на обратной |
...
### Расшифровка маркировок (кратко)

- **MCU `CYT3DLABHBQ1AES`** — TRAVEO II, Cortex-M7, 4160 КБ flash, `HB` = **защита
  включена (HSM + RSA-3K)** — именно это закрывает внутреннюю flash от чтения.
- **NOR `S25HL512TFB01`** ×2 (U17/U18) — SEMPER, 3.0 В, 64 МБ, BGA-24; RDID = `34 2A 1A`.
  64 МБ требуют 4-байтовой адресации при чтении программатором.

> 💡 Ищите доки/софт по базовой части `CYT3DL` (не по полному коду) — по ней материалов
> много и все применимы. Побайтовый разбор маркировок, программаторы, адресная модель —
> канон в [docs/INFINEON_REPOS.md](docs/INFINEON_REPOS.md).

**Итог:** вся память приборки — **1 × CYT3DLABHBQ1AES + 2 × S25HL512TFB01 (по 64 МБ)**.
Графика во внешних NOR (их читаем), таблица привязки ресурсов — во внутренней flash MCU
(закрыта HSM).
...
Лицевая сторона платы с подписанными чипами MCU и NOR flash — в разделе
[«Как выглядит приборка» → «Разобранная плата»](#разобранная-плата-приборки).

### Родственная модель — Geely EX2 (то же железо)

По данным сообщества, приборка и ГУ у **Geely EX2 аппаратно такие же** — полезно как
**справка** (расширяет круг поиска материалов; подтверждает механизм «язык приборки задаёт
ГУ по CAN»). ⚠️ Но **EX2 НЕ годится как донор дампа**: прошивка и графика другие, для `diff`
регионы не совпадут. Подробнее — [docs/GLOBAL_VERSION_MULTILANG.md](docs/GLOBAL_VERSION_MULTILANG.md).
...
В ГУ есть скрытое **инженерное меню** (два разных вида). Проверено: **к приборке доступа не
даёт** — обновление MCU и выгрузка DUMP/логов относятся к самому ГУ, а не к приборке
(Infineon CYT3DL). Снять дамп приборки или залить перевод через него **нельзя**. Полный
разбор обоих меню (перевод пунктов, фото, доказательства) — в
...
### ⛔ Два способа ГАРАНТИРОВАННО (необратимо) окирпичить приборку

1. **Запуск приборки с выпаянным одним из двух NOR-чипов** — не заведётся больше никогда,
   даже после припайки обратно.
2. **Прерывание UDS-заливки на середине** (обрыв связи, разряд ноутбука/аккумулятора,
   выключенное зажигание) — кирпич, повторная заливка по UDS невозможна.

Механизм, правила безопасности и как этого избежать —
[docs/PRACTICAL_APPROACH.md](docs/PRACTICAL_APPROACH.md) → «Гарантированные способы окирпичить приборку».
